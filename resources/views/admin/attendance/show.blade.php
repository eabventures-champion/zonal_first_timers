                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-3">
                    <li class="inline-flex items-center">
                        <a href="{{ route('admin.attendance.index') }}"
                            class="text-xs font-medium text-gray-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-white">
                            Attendance
                        </a>
                    </li>
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span
                                class="ml-1 text-xs font-medium text-gray-700 dark:text-slate-300">{{ $church->name }}</span>
                        </div>
                    </li>
                </ol>
            </nav>
            <p class="text-sm text-gray-500 mb-2">Track weekly church attendance for {{ $church->name }}</p>
            <form action="{{ route('admin.attendance.show', $church) }}" method="GET" class="flex items-center gap-2">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm">
                <thead class="bg-gray-50 dark:bg-slate-800/50">
                    <tr>
                        <th
                            class="px-6 py-4 text-left font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-[11px]">
                            First Timer</th>
                        <th
                            class="px-6 py-4 text-left font-bold text-gray-500 dark:text-slate-400 uppercase tracking-wider text-[11px]">
                            Date Joined</th>
                        @foreach($sundays as $sunday)
                            <th class="px-3 py-4 text-center">
                                <span
                                    class="block text-[11px] font-bold text-gray-500 dark:text-slate-400 uppercase tracking-tight">{{ $sunday['label'] }}</span>
                                <span
                                    class="block text-[9px] text-gray-400 dark:text-slate-500 mt-0.5">{{ \Carbon\Carbon::parse($sunday['date'])->format('M d') }}</span>
                            </th>
                        @endforeach
                    </tr>
                </thead>
                    ...
                    @forelse($groupedAttendance as $joinMonth => $firstTimers)
                        <tr class="bg-gray-50/10 dark:bg-slate-800/20">
                            <td colspan="{{ 2 + count($sundays) }}" class="px-6 py-2">
                                <span
                                    class="text-[10px] font-bold uppercase tracking-wider text-indigo-500/80 dark:text-indigo-400/80">JOINED
                                    IN {{ strtoupper($joinMonth) }}</span>
                            </td>
                        </tr>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        <div
                                            class="w-8 h-8 rounded-full bg-indigo-50 dark:bg-indigo-950/30 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-bold text-xs">
                                            {{ strtoupper(substr($data['name'], 0, 1)) }}
                                        </div>
                                        <div>
                                            <p class="font-bold text-gray-900 dark:text-slate-200 text-sm">{{ $data['name'] }}</p>
                                            <span
                                                class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-md text-[9px] font-bold bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400">
                        ...
                        <tr>
                            <td colspan="{{ 2 + count($sundays) }}" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center gap-4 opacity-30">
                                    ...
                                    <p class="text-lg font-medium text-gray-500 dark:text-slate-400">No attendance records for
                                        newcomers in this church for this period.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
