        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500">{{ $firstTimer->church->name ?? 'No church' }}</p>
                </div>
            </div>
            @php
            ...
            @endphp
            <span
                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {{ $sc[$firstTimer->status] ?? 'bg-gray-100' }}">{{ $firstTimer->status }}</span>
        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-400">
                                    ...
                                    @elseif($item['attended']) Attended — Not yet completed
                                    @else Not attended
                                    @endif
                                </p>
                            </div>
                        ...
                        @if(!$item['completed'] && $firstTimer->status !== 'Member')
                            <form method="POST" action="{{ route('admin.foundation-school.attendance', $firstTimer) }}"
                                ...
                                @if(!$item['attended'])
                                    ...
                                    <button type="submit"
                                        class="px-3 py-1.5 text-xs font-medium text-amber-700 bg-amber-50 hover:bg-amber-100 rounded-lg border border-amber-200 transition">
                                        Mark Attended
                                    </button>
                                @endif
