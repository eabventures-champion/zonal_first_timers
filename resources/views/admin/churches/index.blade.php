    ...
    <div x-data="{ 
        ...
        <div class="bg-white dark:bg-slate-900 p-4 rounded-xl border border-gray-100 dark:border-slate-800 shadow-sm">
            <div class="relative max-w-md">
                ...
                <input type="text" x-model="search" placeholder="Search church, leader or officer..."
                    class="block w-full pl-10 pr-3 py-2 border border-gray-200 dark:border-slate-700 rounded-xl leading-5 bg-gray-50 dark:bg-slate-800 text-gray-900 dark:text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 sm:text-sm transition-all">
            </div>
        </div>

        <div class="space-y-8">
            @forelse($groups as $group)
                <div class="space-y-3" x-show="shouldShowGroup(@js([
                    'name' => $group->name,
                    ...
                    <div x-show="expandedGroups.has('{{ addslashes($group->name) }}') || (search && shouldShowGroup(@js([
                        'name' => $group->name,
                        'churches' => $group->churches->map(fn($c) => [
                            'name' => $c->name,
                            'leader_name' => $c->leader_name,
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm min-w-[650px]">
                                <thead class="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-100 dark:border-slate-800">
                                    <tr>
                                        <th class="px-6 py-3 text-left font-medium text-gray-500 dark:text-slate-400">Church
                                        </th>
                                        <th class="px-6 py-3 text-left font-medium text-gray-500 dark:text-slate-400">Retaining
                                            Officer</th>
                                        <th class="px-6 py-3 text-center font-medium text-gray-500 dark:text-slate-400">First
                                            Timers
                                        </th>
                                        <th class="px-6 py-3 text-center font-medium text-gray-500 dark:text-slate-400">Members
                                        </th>
                                        <th class="px-6 py-3 text-right font-medium text-gray-500 dark:text-slate-400">Actions
                                        </th>
                                    </tr>
                                </thead>
