            ...
            <form method="POST" action="{{ route('admin.churches.update', $church) }}">
                ...
                <div class="mb-5">
                    ...
                    <textarea name="address" id="address" rows="2"
                        class="w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">{{ old('address', $church->address) }}</textarea>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <select name="retaining_officer_id" id="retaining_officer_id"
                        ...
                        @foreach($officers as $officer)
                            <option value="{{ $officer->id }}" {{ old('retaining_officer_id', $church->retaining_officer_id) == $officer->id ? 'selected' : '' }}>{{ $officer->name }}</option>
                        @endforeach
                    </select>
                </div>
