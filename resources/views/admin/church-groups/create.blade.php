    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 p-6">
            <form method="POST" action="{{ route('admin.church-groups.store') }}">
                ...
                <div class="mb-5">
                    <label for="church_category_id"
                        class="block text-sm font-medium text-gray-700 dark:text-slate-400 mb-1">Category <span
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                    <div>
                        ...
                        <input type="text" name="pastor_name" id="pastor_name" value="{{ old('pastor_name') }}"
                            class="w-full rounded-lg border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-white shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        @error('pastor_name') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="pastor_contact"
