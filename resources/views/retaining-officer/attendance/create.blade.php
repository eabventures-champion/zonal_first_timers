    ...
    <div class="max-w-3xl">
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 p-6">
            <form method="POST" action="{{ route('ro.attendance.store') }}">
                ...
                <h3 class="text-sm font-semibold text-gray-700 dark:text-slate-300 mb-3">Mark attendance for each first timer:</h3>
                <div class="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-4 mb-6 space-y-3">
                    @forelse($firstTimers as $ft)
                        <label class="flex items-center gap-3 p-2 rounded-lg hover:bg-white dark:hover:bg-slate-800 cursor-pointer transition">
                            <input type="checkbox" name="attended[{{ $ft->id }}]" value="1"
                                class="rounded border-gray-300 dark:border-slate-700 text-indigo-600 focus:ring-indigo-500 bg-white dark:bg-slate-900">
                            <div>
                    ...
                    @empty
                        <p class="text-sm text-gray-400 dark:text-slate-500">No first timers at your church.</p>
                    @endforelse
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" rows="2"
                        class="w-full rounded-lg border-gray-300 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200 text-sm focus:border-indigo-500 focus:ring-indigo-500">{{ old('notes') }}</textarea>
                </div>
